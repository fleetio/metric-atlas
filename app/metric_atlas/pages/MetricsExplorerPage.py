...
@dataclass
class MetricsExplorerPage:
    metric_categories: list = field(default_factory=lambda: [])

    def __post_init__(self):
        self.metric_categories = Config().metric_categories
    ...
    def render(self):
        ...
        with def_col2:
            ...
            for filter in filters:
                if len(filter["filter_values"]) > 0:
                    st.write(
                        f"**{filter['label']}:** {', '.join(filter['filter_values'])}"
                    )
                    filter_count += 1
        ...
        if len(tab_names) == 2:
            with tabs[1]:
                ...
                col4.metric(
                    "Previous Year",
                    helpers.format_number(
                        standard_metrics["last_year_ptd"], metric.type
                    ),
        ...
        if len(metric.dimensions) >= 1:
            ...
            slice_by_line_chart, slice_by_bar_chart = st.tabs(
            ...
            slice_by_metrics_data, slice_by_raw_data = st.tabs(
                ["Metrics Data", "Raw Data"]
            )
